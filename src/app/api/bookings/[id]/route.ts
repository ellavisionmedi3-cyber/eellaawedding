...
async function sendClientStatusEmail(booking: any, newStatus: string) {
  ...
  if (newStatus === "confirmed") {
    subject = `تأكيد حجز موعد تصوير زفافك - إيلام للفوتوغرافي`;
    htmlContent = `
      <div style="font-family: Arial, sans-serif; direction: rtl; text-align: right; max-width: 600px; margin: 0 auto; border: 1px solid #FFB8CC; border-radius: 10px; padding: 25px; background-color: #fff;">
        ...
        <p style="font-size: 16px; color: #333; line-height: 1.6;">يسعدنا إبلاغك بأنه تم تأكيد حجز موعد تصوير مناسبتك لدى فريق <strong>إيلام للفوتوغرافي</strong> بنجاح بعد اكتمال الدفعة.</p>
        ...
        <p style="font-size: 16px; color: #333; line-height: 1.6;">يتطلع فريقنا لتوثيق أجمل لحظاتك وصنع ذكريات لا تُنسى في يومك المميز. سنقوم بالتواصل معكِ قبل موعد الحفل لمناقشة التفاصيل والترتيبات النهائية.</p>
        ...
        <div style="text-align: center; margin-top: 30px; border-top: 1px solid #ddd; padding-top: 20px;">
          <p style="font-size: 14px; color: #777;">مع كل الحب والتقدير،<br/><strong>فريق إيلام للفوتوغرافي</strong></p>
        </div>
      </div>
    `;
  } else if (newStatus === "cancelled") {
    subject = `إلغاء حجز موعد تصوير مناسبتك - إيلام للفوتوغرافي`;
    htmlContent = `
      <div style="font-family: Arial, sans-serif; direction: rtl; text-align: right; max-width: 600px; margin: 0 auto; border: 1px solid #ccc; border-radius: 10px; padding: 25px; background-color: #fff;">
        ...
        <div style="text-align: center; margin-top: 30px; border-top: 1px solid #ddd; padding-top: 20px;">
          <p style="font-size: 14px; color: #777;"><strong>إيلام للفوتوغرافي</strong></p>
        </div>
      </div>
    `;
  }
  ...
  if (subject && htmlContent) {
    try {
      ...
      await transporter.sendMail({
        from: `"إيلام للفوتوغرافي" <${mailUser}>`,
        to: booking.email,
        subject: subject,
        html: htmlContent
      });
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // If status changed to confirmed or cancelled, send email to client
